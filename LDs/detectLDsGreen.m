function ldCandidatesGreen = detectLDsGreen(imgRGB, nucMask, params, debug)
% detectLDsGreen - Detect Lipid Droplets (LDs) in the green channel using combined DoG and SDOG filtering.
%
% PURPOSE:
%   This function identifies Lipid Droplet candidates in the green channel of a fluorescence image
...
%
% OUTPUT:
%   ldCandidatesGreen : MxN binary mask (logical) of detected LD candidates in the green channel.
%
% EXCEPTIONS:
...
%   ldMaskGreen = detectLDsGreen(img, nucMask, [], true);
%   imshow(ldMaskGreen); title('Detected LDs (Green Channel)');
%
% AUTHOR:
...
if nargin < 3 || isempty(params)
    ...
    params.dog = struct('sigmaSmall', 1, ...
                        'sigmaLarge', 2, ...
...
green = im2double(imgRGB(:,:,2));

% ----------------- 1. DoG Filtering ------------------

greenCLAHE = adapthisteq(green, 'ClipLimit', 0.01, 'NumTiles', [8 8]);
background = imopen(greenCLAHE, strel('disk', 15));
enhanced = imsubtract(greenCLAHE, background);
...
sdogResp = steerableDoGGreen(imgRGB, params.sdog.sigma);
...
statsUnion = regionprops(maskUnionExclusive, green, 'PixelIdxList', 'MeanIntensity');
...
% Final merged mask
ldCandidatesGreen = maskIntersect | maskUnionExclusiveFiltered;

% ----------------- 4. Distance and Background Filtering ------------------

ldCandidatesGreen = filterByDistanceAndBackground(ldCandidatesGreen, nucMask, green, params);

% ----------------- 5. Debug Visualization ------------------

if debug
    figure('Name','Green LD Detection Debug');

    subplot(3,3,1); imshow(green); title('Original Green Channel');
    ...
    subplot(3,3,7); imshow(ldCandidatesGreen); title('Final Combined Mask');
    subplot(3,3,8); imshow(imgRGB); hold on;
    visboundaries(ldCandidatesGreen, 'Color', 'm'); title('Overlay on RGB'); hold off;
end
end

%% ------------------------------------------------------------------------
function sdogResp = steerableDoGGreen(imgRGB, sigma)
% steerableDoGGreen - Computes steerable DoG response in green channel.
%
% PURPOSE:
...
%
% NOTES:
%   - Used internally by detectLDsGree.
...
% Extract green channel
    ...
    Ix = imfilter(green, Gx, 'replicate');
    Iy = imfilter(green, Gy, 'replicate');
    Ixx = imfilter(green, Gxx, 'replicate');
    Iyy = imfilter(green, Gyy, 'replicate');
    Ixy = imfilter(green, Gxy, 'replicate');
...
% Teste 2
% function ldCandidatesGreen = detectLDsGreen(imgRGB, debug, nucMask)
% % detectLDsGreen - Deteta candidatos a LDs verdes com pré-processamento melhorado
% % Entrada:
...
% % Saída:
% %   - ldCandidatesGreen: imagem binária com LDs candidatos
% 
% % --- Extração e pré-processamento ---
% green = im2double(imgRGB(:,:,2));
% greenCLAHE = adapthisteq(green, 'ClipLimit', 0.01, 'NumTiles', [8 8]);
% 
% % Remoção de fundo com top-hat morfológico (mais preciso que gaussiano)
% background = imopen(greenCLAHE, strel('disk', 15));
% enhanced = imsubtract(greenCLAHE, background);
% 
...
% 
% % --- Binarização robusta por percentil (evita oversegmentação) ---
% threshold = prctile(dog(:), 98);  % ajustável
% bw = dog > threshold;
% 
% % --- Pós-processamento morfológico ---
% bw = bwareaopen(bw, 5);  % remove objetos pequenos
% bw = imclose(bw, strel('disk', 1));
% bw = imfill(bw, 'holes');
% 
% % --- Filtro adicional por propriedades morfológicas ---
...
% 
% % --- Remover LDs sobrepostos a núcleos ---
% if nargin > 2 && ~isempty(nucMask)
%     bwFiltered = bwFiltered & ~nucMask;
% 
%     % --- NOVO: Remover LDs demasiado distantes de núcleos ---
%     maxDist = 90;  % distância máxima permitida (em píxeis)
%     D = bwdist(nucMask);
...
% end
% 
% ldCandidatesGreen = bwFiltered;
% 
% % --- Debug visual ---
% if debug
%     figure;
%     subplot(2,4,1); imshow(green); title('Canal Verde Original');
%     subplot(2,4,2); imshow(greenCLAHE); title('CLAHE');
%     subplot(2,4,3); imshow(background); title('Fundo morfológico');
%     subplot(2,4,4); imshow(enhanced); title('Após subtração do fundo');
%     subplot(2,4,5); imshow(dog); title('DoG');
%     subplot(2,4,6); imshow(bw); title('Binarização bruta');
...
% Teste 1
% function ldCandidatesGreen = detectLDsGreen(imgRGB, debug, nucMask)
% % detectLDsGreen - Detecta candidatos a LDs verdes usando DoG + segmentação por círculos
% %
% % Entradas:
% %   - imgRGB: imagem RGB original
...
% %
% % Saída:
% %   - ldCandidatesGreen: imagem binária com candidatos a LDs verdes
% 
% % --- 1. Pré-processamento ---
% green = im2double(imgRGB(:,:,2));
% greenCLAHE = adapthisteq(green, 'ClipLimit', 0.03, 'NumTiles', [8 8]);
% 
% background = imopen(greenCLAHE, strel('disk', 15));
% enhanced = imsubtract(greenCLAHE, background);
% 
...
% end
% 
% % --- Função auxiliar segmentImage (mesma que antes) ---
